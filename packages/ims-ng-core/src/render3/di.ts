...
export function getOrCreateNodeInjectorForNode(
  ...
  if (tView.firstTemplatePass) {
    ...
    ngDevMode &&
      assertEqual(
        tNode.flags === 0 || tNode.flags === TNodeFlags.isComponent,
        true,
 ...
 */
export function getParentInjectorLocation(
  tNode: TNode,
  view: LView,
): RelativeInjectorLocation {
  ...
  return hostTNode
    ? hostTNode.injectorIndex |
        (viewOffset << RelativeInjectorLocationFlags.ViewOffsetShift)
    : (-1 as any);
}
...
export function getOrCreateInjectable<T>(
  ...
  if (typeof bloomHash === 'function') {
    ...
    let hostTElementNode: TNode | null =
      flags & InjectFlags.Host ? getHostTElementNode(lView) : null;
...
export function locateDirectiveOrProvider<T>(
  ...
  const injectablesStart =
    nodeProviderIndexes & TNodeProviderIndexes.ProvidersStartIndexMask;
  ...
  for (let i = startingIndex; i < endIndex; i++) {
    ...
    if (
      ...
      (i >= directivesStart &&
        (providerTokenOrDef as DirectiveDef<any>).type === token)
    ) {
      return i;
    }
  }
...
export function injectInjector() {
  const tNode = getPreviousOrParentTNode() as
    | TElementNode
    | TContainerNode
    | TElementContainerNode;
...
export function getFactoryOf<T>(
  type: Type<any>,
): ((type: Type<T> | null) => T) | null {
  const typeAny = type as any;
  const def =
    getComponentDef<T>(typeAny) ||
    getDirectiveDef<T>(typeAny) ||
    getPipeDef<T>(typeAny) ||
    getInjectableDef<T>(typeAny) ||
    getInjectorDef<T>(typeAny);
