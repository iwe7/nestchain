...
export function setCurrentDirectiveDef(
  def: DirectiveDef<any> | ComponentDef<any> | null,
): void {
...
export function getOrCreateCurrentQueries(QueryType: {
  ...
  if (
    ...
    previousOrParentTNode !== lView[HOST_NODE] &&
    !isContentQueryHost(previousOrParentTNode)
  ) {
    currentQueries &&
      (currentQueries = lView[QUERIES] = currentQueries.clone());
...
export function enterView(
  newView: LView,
  hostTNode: TElementNode | TViewNode | null,
): LView {
  const oldView = lView;
  if (newView) {
