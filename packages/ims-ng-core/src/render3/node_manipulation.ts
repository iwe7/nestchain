...
export function getParentNative(
  tNode: TNode,
  currentView: LView,
): RElement | RComment | null {
...
export function getLContainer(
  ...
  if (tNode.index === -1) {
    ...
    return containerHostIndex > -1
      ? embeddedView[PARENT]![containerHostIndex]
      : null;
  } else {
 ...
 */
export function getContainerRenderParent(
  tViewNode: TViewNode,
  view: LView,
): RElement | null {
  ...
  while (tNode) {
    ...
    if (tNode.type === TNodeType.Element) {
      executeNodeAction(
        ...
        renderer,
        renderParent,
        getNativeByTNode(tNode, currentView),
        beforeNode,
      );
...
function executeNodeAction(
  ...
  node: RComment | RElement | RText,
  beforeNode?: RNode | null,
) {
  if (action === WalkTNodeTreeAction.Insert) {
    isProceduralRenderer(renderer!)
      ? (renderer as ProceduralRenderer3).insertBefore(
          parent!,
          node,
          beforeNode as RNode | null,
        )
...
export function addRemoveViewFromContainer(
  ...
  const renderParent = getContainerRenderParent(
    viewToWalk[TVIEW].node as TViewNode,
    viewToWalk,
  );
  ...
  if (renderParent) {
    ...
    walkTNodeTree(
      viewToWalk,
      insertMode ? WalkTNodeTreeAction.Insert : WalkTNodeTreeAction.Detach,
      renderer,
      renderParent,
      beforeNode,
    );
...
export function removeView(
  lContainer: LContainer,
  containerHost: TElementNode | TContainerNode | TElementContainerNode,
  removeIndex: number,
) {
...
export function getParentState(
  state: LView | LContainer,
  rootView: LView,
): LView | LContainer | null {
  let tNode;
  if (
    state.length >= HEADER_OFFSET &&
    (tNode = (state as LView)![HOST_NODE]) &&
...
function cleanUpView(viewOrContainer: LView | LContainer): void {
  if ((viewOrContainer as LView).length >= HEADER_OFFSET) {
    ...
    if (
      hostTNode &&
      hostTNode.type === TNodeType.Element &&
...
export function getRenderParent(
  tNode: TNode,
  currentView: LView,
): RElement | null {
  if (canInsertNativeNode(tNode, currentView)) {
    ...
    if (
      tNodeParent != null &&
      tNodeParent.type === TNodeType.ElementContainer
    ) {
...
function canInsertNativeChildOfElement(tNode: TNode): boolean {
  ...
  if (
    ...
    (tNode.parent.type === TNodeType.Element &&
      !(tNode.parent.flags & TNodeFlags.isComponent))
  ) {
    return true;
  }
 ...
 */
function canInsertNativeChildOfView(
  viewTNode: TViewNode,
  view: LView,
): boolean {
 ...
 */
export function nativeParentNode(
  renderer: Renderer3,
  node: RNode,
): RElement | null {
  return (isProceduralRenderer(renderer)
    ? renderer.parentNode(node)
    : node.parentNode) as RElement;
}

/**
 * Returns a native sibling of a given native node.
 */
export function nativeNextSibling(
  renderer: Renderer3,
  node: RNode,
): RNode | null {
  return isProceduralRenderer(renderer)
...
export function appendChild(
  ...
  if (childEl !== null && canInsertNativeNode(childTNode, currentView)) {
    ...
    if (parentTNode.type === TNodeType.View) {
      const lContainer = getLContainer(
        parentTNode as TViewNode,
        currentView,
      ) as LContainer;
      const views = lContainer[VIEWS];
      ...
      const icuAnchorNode = getNativeByTNode(
        childTNode.parent!,
        currentView,
      )! as RElement;
      nativeInsertBefore(
        renderer,
        parentEl as RElement,
        childEl,
        icuAnchorNode,
      );
    } else {
      isProceduralRenderer(renderer)
        ? renderer.appendChild(parentEl! as RElement, childEl)
...
function getHighestElementContainer(ngContainer: TNode): TNode {
  while (
    ngContainer.parent != null &&
    ngContainer.parent.type === TNodeType.ElementContainer
  ) {
...
export function getBeforeNodeForView(
  index: number,
  views: LView[],
  containerNative: RComment,
) {
  if (index + 1 < views.length) {
    const view = views[index + 1] as LView;
    const viewTNode = view[HOST_NODE] as TViewNode;
    return viewTNode.child
      ? getNativeByTNode(viewTNode.child, view)
      : containerNative;
  } else {
...
export function removeChild(
  ...
  if (childEl !== null && canInsertNativeNode(childTNode, currentView)) {
    ...
    const renderer = currentView[RENDERER];
    isProceduralRenderer(renderer)
      ? renderer.removeChild(parentNative as RElement, childEl)
...
export function appendProjectedNode(
  projectedTNode: TNode,
  tProjectionNode: TNode,
  currentView: LView,
