...
export function assertNgModuleType(
  actual: any,
  msg: string = "Type passed in is not NgModuleType, it does not have 'ngModuleDef' property.",
) {
...
export function assertDomNode(node: any) {
  assertEqual(
    ...
    true,
    'The provided value must be an instance of a DOM Node',
  );
}
...
export function assertDataInRange(arr: any[], index: number) {
  assertLessThan(
    index,
    arr ? arr.length : 0,
    'index expected to be a valid data index',
  );
}
