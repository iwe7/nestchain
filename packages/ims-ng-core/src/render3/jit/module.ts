...
export function compileNgModuleDefs(
  ...
  const declarations: Type<any>[] = flatten(
    ngModule.declarations || EMPTY_ARRAY,
  );
    ...
    get: () => {
      if (ngModuleDef === null) {
        ngModuleDef = getCompilerFacade().compileNgModule(
          angularCoreEnv,
          `ng://${moduleType.name}/ngModuleDef.js`,
          {
            type: moduleType,
            bootstrap: flatten(ngModule.bootstrap || EMPTY_ARRAY),
            declarations: declarations,
            imports: flatten(ngModule.imports || EMPTY_ARRAY).map(
              expandModuleWithProviders,
            ),
            exports: flatten(ngModule.exports || EMPTY_ARRAY).map(
              expandModuleWithProviders,
            ),
            emitInline: true,
          },
  ...
  Object.defineProperty(moduleType, NG_INJECTOR_DEF, {
    get: () => {
      if (ngInjectorDef === null) {
        ngDevMode &&
          verifySemanticsOfNgModuleDef((moduleType as any) as NgModuleType);
        ...
        ngInjectorDef = getCompilerFacade().compileInjector(
          angularCoreEnv,
          `ng://${moduleType.name}/ngInjectorDef.js`,
          meta,
        );
      }
...
function verifySemanticsOfNgModuleDef(moduleType: NgModuleType): void {
  ...
  ngModuleDef.declarations.forEach(
    verifyComponentEntryComponentsIsPartOfNgModule,
  );

  const ngModule = getAnnotation<NgModule>(moduleType, 'NgModule');
  if (ngModule) {
    ngModule.imports &&
      flatten(ngModule.imports, unwrapModuleWithProvidersImports).forEach(
        verifySemanticsOfNgModuleDef,
      );
    ngModule.bootstrap &&
      ngModule.bootstrap.forEach(verifyComponentIsPartOfNgModule);
    ngModule.entryComponents &&
      ngModule.entryComponents.forEach(verifyComponentIsPartOfNgModule);
  }
    ...
    if (!def) {
      errors.push(
        `Unexpected value '${stringify(
          type,
        )}' declared by the module '${stringify(
          moduleType,
  ...
  function verifyExportsAreDeclaredOrReExported(type: Type<any>) {
    ...
    if (kind) {
      ...
      if (combinedDeclarations.lastIndexOf(type) === -1) {
        ...
        errors.push(
          `Can't export ${kind} ${stringify(type)} from ${stringify(
            moduleType,
  ...
  function verifyDeclarationIsUnique(type: Type<any>) {
    ...
    if (existingModule && existingModule !== moduleType) {
      ...
      errors.push(
        `Type ${stringify(type)} is part of the declarations of 2 modules: ${
          modules[0]
        } and ${modules[1]}! ` +
          `Please consider moving ${stringify(
            type,
          )} to a higher module that imports ${modules[0]} and ${
            modules[1]
          }. ` +
...
function setScopeOnDeclaredComponents(
  ...
  declarations.forEach(declaration => {
    if (declaration.hasOwnProperty(NG_COMPONENT_DEF)) {
      ...
      const component = declaration as Type<any> & {
        ngComponentDef: ComponentDef<any>;
      };
      const componentDef = getComponentDef(component)!;
      patchComponentDefWithScope(componentDef, transitiveScopes);
    } else if (
      !declaration.hasOwnProperty(NG_DIRECTIVE_DEF) &&
      !declaration.hasOwnProperty(NG_PIPE_DEF)
    ) {
      // Set `ngSelectorScope` for future reference when the component compilation finishes.
      (declaration as Type<any> & {
        ngSelectorScope?: any;
      }).ngSelectorScope = moduleType;
    }
  });
}
...
export function transitiveScopesFor<T>(
  ...
  def.imports.forEach(<I>(imported: Type<I>) => {
    ...
    if (!isNgModule<I>(importedTyped)) {
      throw new Error(
        `Importing ${importedTyped.name} which does not have an ngModuleDef`,
      );
    }
...
function isNgModule<T>(
  value: Type<T>,
): value is Type<T> & { ngModuleDef: NgModuleDef<T> } {
