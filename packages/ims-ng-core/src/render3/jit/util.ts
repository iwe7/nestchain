...
export function reflectDependencies(
  type: Type<any>,
): R3DependencyMetadataFacade[] {
  ...
  if (Array.isArray(dep)) {
    ...
    for (let j = 0; j < dep.length; j++) {
      ...
      if (param === undefined) {
        ...
        param.__proto__.ngMetadataName === 'Optional'
      ) {
        meta.optional = true;
      } else if (
        param instanceof SkipSelf ||
        param.__proto__.ngMetadataName === 'SkipSelf'
      ) {
        meta.skipSelf = true;
      } else if (
        param instanceof Self ||
        param.__proto__.ngMetadataName === 'Self'
      ) {
        meta.self = true;
      } else if (
        param instanceof Host ||
        param.__proto__.ngMetadataName === 'Host'
      ) {
        meta.host = true;
