...
import {
  DirectiveOwnerAndPlayerBuilderIndex,
  DirectiveRegistryValues,
  DirectiveRegistryValuesIndex,
  InitialStylingValues,
  InitialStylingValuesIndex,
  SinglePropOffsetValues,
  SinglePropOffsetValuesIndex,
  StylingContext,
  StylingFlags,
  StylingIndex,
...
export function initializeStaticContext(attrs: TAttributes) {
  ...
  const initialClasses: InitialStylingValues = (context[
    StylingIndex.InitialClassValuesPosition
  ] = [null]);
  const initialStyles: InitialStylingValues = (context[
    StylingIndex.InitialStyleValuesPosition
  ] = [null]);
  ...
  if (getDirectiveRegistryValuesIndexOf(directives, directive) == -1) {
    ...
    for (let i = 0; i < attrs.length; i++) {
      ...
      if (typeof attr == 'number') {
        ...
        initialStyles =
          initialStyles || context[StylingIndex.InitialStyleValuesPosition];
...
function patchInitialStylingValue(
  initialStyling: InitialStylingValues,
  prop: string,
  value: any,
): void {
  // Even values are keys; Odd numbers are values; Search keys only
  for (
    let i = InitialStylingValuesIndex.KeyValueStartPosition;
    i < initialStyling.length;

  ) {
    const key = initialStyling[i];
    if (key === prop) {
      const existingValue =
        initialStyling[i + InitialStylingValuesIndex.ValueOffset];
...
function renderInitialStylingValues(
  element: RElement,
  renderer: Renderer3,
  initialStylingValues: InitialStylingValues,
  isEntryClassBased: boolean,
) {
  for (
    let i = InitialStylingValuesIndex.KeyValueStartPosition;
    i < initialStylingValues.length;
    i += InitialStylingValuesIndex.Size
  ) {
    const value =
      initialStylingValues[i + InitialStylingValuesIndex.ValueOffset];
    if (value) {
      if (isEntryClassBased) {
        setClass(
          element,
          initialStylingValues[
            i + InitialStylingValuesIndex.PropOffset
          ] as string,
          true,
          renderer,
          null,
...
export function allowNewBindingsForStylingContext(
  context: StylingContext,
): boolean {
  return (
    (context[StylingIndex.MasterFlagPosition] &
      StylingFlags.BindingAllocationLocked) ===
    0
  );
}
...
export function updateContextWithBindings(
  ...
  // are inserted in.
  const singlePropOffsetValues =
    context[StylingIndex.SinglePropOffsetPositions];
  ...
  if (styleBindingNames && styleBindingNames.length) {
    for (let i = 0; i < styleBindingNames.length; i++) {
      ...
      let singlePropIndex = getMatchingBindingIndex(
        context,
        name,
        singleStylesStartIndex,
  ...
  if (filteredStyleBindingNames.length) {
    while (i < currentSinglePropsLength) {
      const totalStyles =
        singlePropOffsetValues[
          i + SinglePropOffsetValuesIndex.StylesCountPosition
        ];
      const totalClasses =
        singlePropOffsetValues[
          i + SinglePropOffsetValuesIndex.ClassesCountPosition
        ];
      if (totalClasses) {
        const start =
          i + SinglePropOffsetValuesIndex.ValueStartPosition + totalStyles;
        for (let j = start; j < start + totalClasses; j++) {
          singlePropOffsetValues[j] +=
            filteredStyleBindingNames.length * StylingIndex.Size;
        }
      }
  ...
  const totalNewEntries =
    filteredClassBindingNames.length + filteredStyleBindingNames.length;

  // in the event that there are new style values being inserted, all existing class and style
  // bindings need to have their pointer values offsetted with the new amount of space that is
  // used for the new style/class bindings.
  for (
    let i = singleStylesStartIndex;
    i < context.length;
    i += StylingIndex.Size
  ) {
    const isMultiBased = i >= multiStylesStartIndex;
    const isClassBased =
      i >= (isMultiBased ? multiClassesStartIndex : singleClassesStartIndex);
    const flag = getPointers(context, i);
    const staticIndex = getInitialIndex(flag);
    let singleOrMultiIndex = getMultiOrSingleIndex(flag);
    if (isMultiBased) {
      singleOrMultiIndex += isClassBased
        ? filteredStyleBindingNames.length * StylingIndex.Size
        : 0;
    } else {
      singleOrMultiIndex +=
        totalNewEntries * StylingIndex.Size +
        (isClassBased ? filteredStyleBindingNames.length : 0) *
          StylingIndex.Size;
    }
  ...
  for (
    let i = 0;
    i < filteredStyleBindingNames.length * StylingIndex.Size;
    i++
  ) {
  ...
  for (
    let i = 0;
    i < filteredClassBindingNames.length * StylingIndex.Size;
    i++
  ) {
  ...
  for (let i = 0; i < totalNewEntries; i++) {
    ...
    const adjustedIndex = entryIsClassBased
      ? i - filteredStyleBindingNames.length
      : i;
    const propName = entryIsClassBased
      ? filteredClassBindingNames[adjustedIndex]
    ...
    let initialValuesToLookup = entryIsClassBased
      ? initialClasses
      : initialStyles;
    let indexForInitial = getInitialStylingValuesIndexOf(
      initialValuesToLookup,
      propName,
    );
    if (indexForInitial === -1) {
      indexForInitial =
        initialValuesToLookup.length + InitialStylingValuesIndex.ValueOffset;
      initialValuesToLookup.push(propName, entryIsClassBased ? false : null);
    } else {
      indexForInitial += InitialStylingValuesIndex.ValueOffset;
    }

    const initialFlag = prepareInitialFlag(
      context,
      propName,
      entryIsClassBased,
      styleSanitizer || null,
    ...
    setFlag(
      context,
      singleIndex,
      pointers(initialFlag, indexForInitial, multiIndex),
    );
    ...
    setFlag(
      context,
      multiIndex,
      pointers(initialFlag, indexForInitial, singleIndex),
    );
  ...
  const masterFlag =
    pointers(0, 0, multiStylesStartIndex) |
...
function findOrPatchDirectiveIntoRegistry(
  ...
  const nextOffsetInsertionIndex =
    context[StylingIndex.SinglePropOffsetPositions].length;

  let directiveIndex: number;
  const detectedIndex = getDirectiveRegistryValuesIndexOf(
    directiveRefs,
    directiveRef,
  );
...
export function updateStylingMap(
  ...
  classesInput:
    | { [key: string]: any }
    | string
    | BoundPlayerFactory<null | string | { [key: string]: any }>
    | NO_CHANGE
    | null,
  stylesInput?:
    | { [key: string]: any }
    | BoundPlayerFactory<null | { [key: string]: any }>
    | NO_CHANGE
    | null,
  directiveRef?: any,
): void {
  ...
  const directiveIndex = getDirectiveIndexFromRegistry(
    context,
    directiveRef || null,
  );
  const element = context[StylingIndex.ElementPosition]! as HTMLElement;
  const classesPlayerBuilder =
    classesInput instanceof BoundPlayerFactory
      ...
      : null;
  const stylesPlayerBuilder =
    stylesInput instanceof BoundPlayerFactory
      ? new ClassAndStylePlayerBuilder(
          stylesInput as any,
          element,
          BindingType.Style,
  ...
  const classesValue = classesPlayerBuilder
    ? (classesInput as BoundPlayerFactory<{ [key: string]: any } | string>)!
        .value
    : classesInput;
  const stylesValue = stylesPlayerBuilder ? stylesInput!.value : stylesInput;
  // early exit (this is what's done to avoid using ctx.bind() to cache the value)
  const ignoreAllClassUpdates =
    limitToSingleClasses(context) ||
    classesValue === NO_CHANGE ||
    classesValue === context[StylingIndex.CachedClassValueOrInitialClassString];
  const ignoreAllStyleUpdates =
    stylesValue === NO_CHANGE ||
    stylesValue === context[StylingIndex.CachedStyleValue];
  ...
  const classesPlayerBuilderIndex = classesPlayerBuilder
    ? PlayerIndex.ClassMapPlayerBuilderPosition
    : 0;
  if (
    hasPlayerBuilderChanged(
      context,
      classesPlayerBuilder,
      PlayerIndex.ClassMapPlayerBuilderPosition,
    )
  ) {
    setPlayerBuilder(
  ...
  const stylesPlayerBuilderIndex = stylesPlayerBuilder
  ...
  while (ctxIndex < context.length && propIndex < propLimit) {
    ...
    const processValue =
      (!isClassBased && !ignoreAllStyleUpdates) ||
      (isClassBased && !ignoreAllClassUpdates);

    // when there is a cache-hit for a string-based class then we should
    // avoid doing any work diffing any of the changes
    if (processValue) {
      const adjustedPropIndex = isClassBased
        ? propIndex - classesStartIndex
        : propIndex;
      const newProp: string = isClassBased
        ? classNames[adjustedPropIndex]
        : styleProps[adjustedPropIndex];
      const newValue: string | boolean = isClassBased
        ? applyAllClasses
          ? true
          : classes[newProp]
        : styles[newProp];
      const playerBuilderIndex = isClassBased
        ? classesPlayerBuilderIndex
        : stylesPlayerBuilderIndex;

      const prop = getProp(context, ctxIndex);
      if (prop === newProp) {
        const value = getValue(context, ctxIndex);
        const flag = getPointers(context, ctxIndex);
        setPlayerBuilderIndex(
          context,
          ctxIndex,
          playerBuilderIndex,
          directiveIndex,
        ...
        if (hasValueChanged(flag, value, newValue)) {
          ...
          playerBuildersAreDirty =
            playerBuildersAreDirty || !!playerBuilderIndex;
        ...
        const indexOfEntry = findEntryPositionByProp(
          context,
          newProp,
          ctxIndex,
        );
        if (indexOfEntry > 0) {
          ...
          if (hasValueChanged(flagToCompare, valueToCompare, newValue)) {
            ...
            if (
              valueToCompare != null ||
              hasValueChanged(flagToCompare, initialValue, newValue)
            ) {
              setDirty(context, ctxIndex, true);
              playerBuildersAreDirty =
                playerBuildersAreDirty || !!playerBuilderIndex;
          ...
          const newFlag = prepareInitialFlag(
            ...
            newProp,
            isClassBased,
            getStyleSanitizer(context, directiveIndex),
          );
          playerBuildersAreDirty =
            playerBuildersAreDirty || !!playerBuilderIndex;
  ...
  while (propIndex < propLimit) {
    ...
    if (processValue) {
      const adjustedPropIndex = isClassBased
        ? propIndex - classesStartIndex
        : propIndex;
      const prop = isClassBased
        ? classNames[adjustedPropIndex]
        : styleProps[adjustedPropIndex];
      const value: string | boolean = isClassBased
        ? applyAllClasses
          ? true
          : classes[prop]
        : styles[prop];
      const flag =
        prepareInitialFlag(context, prop, isClassBased, sanitizer) |
        StylingFlags.Dirty;
      const playerBuilderIndex = isClassBased
        ? classesPlayerBuilderIndex
        : stylesPlayerBuilderIndex;
...
function _updateSingleStylingValue(
  ...
  const singleIndex = getSinglePropIndexValue(
    context,
    directiveIndex,
    offset,
    isClassBased,
  ...
  const value: string | boolean | null =
    input instanceof BoundPlayerFactory ? input.value : input;

  if (
    hasValueChanged(currFlag, currValue, value) &&
    ...
    const element = context[StylingIndex.ElementPosition]! as HTMLElement;
    const playerBuilder =
      input instanceof BoundPlayerFactory
        ? new ClassAndStylePlayerBuilder(
            input as any,
            element,
        ...
        : null;
    const value = (playerBuilder
      ? (input as BoundPlayerFactory<any>).value
      : input) as string | boolean | null;
    ...
    if (hasPlayerBuilderChanged(context, playerBuilder, currPlayerIndex)) {
      const newIndex = setPlayerBuilder(
        context,
        playerBuilder,
        currPlayerIndex,
      );
    ...
    if (currDirective !== directiveIndex) {
      ...
      setSanitizeFlag(
        context,
        singleIndex,
        sanitizer && sanitizer(prop) ? true : false,
      );
    }
    ...
    if (!valueForMulti || hasValueChanged(currFlag, valueForMulti, value)) {
      ...
      if (
        !valueExists(value, isClassBased) &&
        valueExists(valueForMulti, isClassBased)
      ) {
        multiDirty = true;
        singleDirty = false;
      }
...
export function renderStyling(
  context: StylingContext,
  renderer: Renderer3,
  rootOrView: RootContext | LView,
  isFirstRender: boolean,
  classesStore?: BindingStore | null,
  stylesStore?: BindingStore | null,
  directiveRef?: any,
): number {
  let totalPlayersQueued = 0;
  const targetDirectiveIndex = getDirectiveIndexFromRegistry(
    context,
    directiveRef || null,
  );
  ...
  if (
    ...
    isDirectiveDirty(context, targetDirectiveIndex)
  ) {
    const flushPlayerBuilders: any =
      context[StylingIndex.MasterFlagPosition] &
      StylingFlags.PlayerBuildersDirty;
    ...
    for (
      let i = StylingIndex.SingleStylesStartPosition;
      i < context.length;
      i += StylingIndex.Size
    ) {
      ...
      if (isDirty(context, i)) {
        ...
        if (
          !isClassBased &&
          !valueExists(valueToApply, isClassBased) &&
          readInitialValue
        ) {
    ...
    if (flushPlayerBuilders) {
      const rootContext = Array.isArray(rootOrView)
        ? getRootContext(rootOrView)
        : (rootOrView as RootContext);
      const playerContext = getPlayerContext(context)!;
      const playersStartIndex =
        playerContext[PlayerIndex.NonBuilderPlayersStart];
      for (
        let i = PlayerIndex.PlayerBuildersStartPosition;
        i < playersStartIndex;
        i += PlayerIndex.PlayerAndPlayerBuildersTupleSize
      ) {
        const builder = playerContext[i] as ClassAndStylePlayerBuilder<
          any
        > | null;
...
function setDirty(context: StylingContext, index: number, isDirtyYes: boolean) {
  const adjustedIndex =
    index >= StylingIndex.SingleStylesStartPosition
      ? index + StylingIndex.FlagsOffset
      : index;
  if (isDirtyYes) {
...
export function isClassBasedValue(
  context: StylingContext,
  index: number,
): boolean {
  const adjustedIndex =
    index >= StylingIndex.SingleStylesStartPosition
      ...
      : index;
  return (
    ((context[adjustedIndex] as number) & StylingFlags.Class) ==
    StylingFlags.Class
  );
}
...
function isSanitizable(context: StylingContext, index: number): boolean {
  const adjustedIndex =
    index >= StylingIndex.SingleStylesStartPosition
      ...
      : index;
  return (
    ((context[adjustedIndex] as number) & StylingFlags.Sanitize) ==
...
function pointers(
  ...
  return (
    (configFlag & StylingFlags.BitMask) |
    (staticIndex << StylingFlags.BitCountSize) |
...
function getInitialValue(
  ...
  const initialValues = entryIsClassBased
    ? context[StylingIndex.InitialClassValuesPosition]
...
function hasPlayerBuilderChanged(
  context: StylingContext,
  builder: ClassAndStylePlayerBuilder<any> | null,
  index: number,
) {
...
function setPlayerBuilder(
  ...
  let playerContext =
    context[StylingIndex.PlayerContext] || allocPlayerContext(context);
  if (insertionIndex > 0) {
...
export function directiveOwnerPointers(
  ...
  return (
    (playerIndex << DirectiveOwnerAndPlayerBuilderIndex.BitCountSize) |
    directiveIndex
  );
}
...
function getPlayerBuilderIndex(context: StylingContext, index: number): number {
  ...
  const playerBuilderIndex =
    (flag >> DirectiveOwnerAndPlayerBuilderIndex.BitCountSize) &
    DirectiveOwnerAndPlayerBuilderIndex.BitMask;
  return playerBuilderIndex;
}

function getPlayerBuilder(
  context: StylingContext,
  index: number,
): ClassAndStylePlayerBuilder<any> | null {
  const playerBuilderIndex = getPlayerBuilderIndex(context, index);
  if (playerBuilderIndex) {
    const playerContext = context[StylingIndex.PlayerContext];
    if (playerContext) {
      return playerContext[playerBuilderIndex] as ClassAndStylePlayerBuilder<
        any
      > | null;
    }
  }
...
function setFlag(context: StylingContext, index: number, flag: number) {
  const adjustedIndex =
    index === StylingIndex.MasterFlagPosition
      ? index
      : index + StylingIndex.FlagsOffset;
...
export function limitToSingleClasses(context: StylingContext) {
  return (
    context[StylingIndex.MasterFlagPosition] &
    StylingFlags.OnlyProcessSingleClasses
  );
}

export function setContextDirty(
  context: StylingContext,
  isDirtyYes: boolean,
): void {
...
export function setContextPlayersDirty(
  context: StylingContext,
  isDirtyYes: boolean,
): void {
  if (isDirtyYes) {
    (context[
      StylingIndex.MasterFlagPosition
    ] as number) |= StylingFlags.PlayerBuildersDirty;
  } else {
    (context[
      StylingIndex.MasterFlagPosition
    ] as number) &= ~StylingFlags.PlayerBuildersDirty;
...
function findEntryPositionByProp(
  ...
  for (
    let i = (startIndex || 0) + StylingIndex.PropertyOffset;
    i < context.length;
    i += StylingIndex.Size
  ) {
...
function swapMultiContextEntries(
  context: StylingContext,
  indexA: number,
  indexB: number,
) {
  ...
  setPlayerBuilderIndex(
    context,
    indexB,
    tmpPlayerBuilderIndex,
    directiveIndexA,
  );
}

function updateSinglePointerValues(
  context: StylingContext,
  ...
  for (let i = indexStartPosition; i < context.length; i += StylingIndex.Size) {
    ...
    if (singleIndex > 0) {
      ...
      const flagValue =
        (isDirty(context, singleIndex)
          ? StylingFlags.Dirty
          : StylingFlags.None) |
        (isClassBasedValue(context, singleIndex)
          ? StylingFlags.Class
          : StylingFlags.None) |
        (isSanitizable(context, singleIndex)
          ? StylingFlags.Sanitize
          : StylingFlags.None);
...
function insertNewMultiProperty(
  ...
  context.splice(
    ...
    flag |
      StylingFlags.Dirty |
      (classBased ? StylingFlags.Class : StylingFlags.None),
    name,
    value,
    0,
  );
...
function prepareInitialFlag(
  context: StylingContext,
  prop: string,
  entryIsClassBased: boolean,
  sanitizer?: StyleSanitizeFn | null,
) {
  let flag =
    sanitizer && sanitizer(prop) ? StylingFlags.Sanitize : StylingFlags.None;

  let initialIndex: number;
  if (entryIsClassBased) {
    flag |= StylingFlags.Class;
    initialIndex = getInitialStylingValuesIndexOf(
      context[StylingIndex.InitialClassValuesPosition],
      prop,
    );
  } else {
  ...
  initialIndex =
    initialIndex > 0 ? initialIndex + InitialStylingValuesIndex.ValueOffset : 0;
...
export function generateConfigSummary(
  source: StylingContext,
  index: number,
): LogSummary;
export function generateConfigSummary(
  source: number | StylingContext,
  index?: number,
): LogSummary {
  let flag,
    name = 'config value for ';
  ...
  return {
    ...
    flags: {
      ...
      playerBuildersDirty:
        flag & StylingFlags.PlayerBuildersDirty ? true : false,
      onlyProcessSingleClasses:
        flag & StylingFlags.OnlyProcessSingleClasses ? true : false,
      bindingAllocationLocked:
        flag & StylingFlags.BindingAllocationLocked ? true : false,
    },
  };
}

export function getDirectiveIndexFromEntry(
  context: StylingContext,
  index: number,
) {
  const value = context[
    index + StylingIndex.PlayerBuilderIndexOffset
  ] as number;
...
function getDirectiveIndexFromRegistry(
  ...
  const index = getDirectiveRegistryValuesIndexOf(
...
function getDirectiveRegistryValuesIndexOf(
  directives: DirectiveRegistryValues,
  directive: {},
): number {
  for (
    let i = 0;
    i < directives.length;
    i += DirectiveRegistryValuesIndex.Size
  ) {
...
function getInitialStylingValuesIndexOf(
  keyValues: InitialStylingValues,
  key: string,
): number {
  for (
    let i = InitialStylingValuesIndex.KeyValueStartPosition;
...
export function compareLogSummaries(a: LogSummary, b: LogSummary) {
  ...
  Object.keys(a.flags).forEach(name => {
    diffSummaryValues(diffs, 'flags.' + name, name, a.flags, b.flags);
  });
...
function getSinglePropIndexValue(
  ...
  const singlePropOffsetRegistryIndex = context[
    StylingIndex.DirectiveRegistryPosition
  ][
  ...
  const indexForOffset =
    singlePropOffsetRegistryIndex +
    SinglePropOffsetValuesIndex.ValueStartPosition +
    (isClassBased
      ? offsets[
          singlePropOffsetRegistryIndex +
            SinglePropOffsetValuesIndex.StylesCountPosition
        ]
      : 0) +
    offset;
...
function getStyleSanitizer(
  context: StylingContext,
  directiveIndex: number,
): StyleSanitizeFn | null {
  ...
  const value =
    ...
    dirs[DirectiveRegistryValuesIndex.StyleSanitizerOffset] ||
    null;
  return value as StyleSanitizeFn | null;
}

function isDirectiveDirty(
  context: StylingContext,
  directiveIndex: number,
): boolean {
  const dirs = context[StylingIndex.DirectiveRegistryPosition];
  return dirs[
    directiveIndex * DirectiveRegistryValuesIndex.Size +
      DirectiveRegistryValuesIndex.DirtyFlagOffset
  ] as boolean;
}
...
function allowValueChange(
  ...
  currentDirectiveOwner: number,
  newDirectiveOwner: number,
) {
