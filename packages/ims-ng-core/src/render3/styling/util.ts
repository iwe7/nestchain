...
export function createEmptyStylingContext(
  ...
  initialClasses?: InitialStylingValues | null,
): StylingContext {
  return [
    0, // MasterFlags
...
export function allocStylingContext(
  element: RElement | null,
  templateStyleContext: StylingContext,
): StylingContext {
 ...
 */
export function getStylingContext(
  index: number,
  viewData: LView,
): StylingContext {
  let storageIndex = index;
  let slotValue: LContainer | LView | StylingContext | RElement =
    viewData[storageIndex];
  ...
  if (isStylingContext(wrapper)) {
    ...
    const stylingTemplate = getTNode(index - HEADER_OFFSET, viewData)
      .stylingTemplate;
...
export function isStylingContext(value: any): value is StylingContext {
  ...
  return (
    Array.isArray(value) &&
    typeof value[StylingIndex.MasterFlagPosition] === 'number' &&
...
export function addPlayerInternal(
  ...
  if (player) {
    ...
    const playerHandler =
      rootContext.playerHandler ||
      (rootContext.playerHandler = new CorePlayerHandler());
...
export function getPlayersInternal(playerContext: PlayerContext): Player[] {
  ...
  const nonFactoryPlayersStart =
    playerContext[PlayerIndex.NonBuilderPlayersStart];

  // add all factory-based players (which are apart of [style] and [class] bindings)
  for (
    let i =
      PlayerIndex.PlayerBuildersStartPosition +
      PlayerIndex.PlayerOffsetPosition;
    i < nonFactoryPlayersStart;
...
export function getOrCreatePlayerContext(
  target: {},
...
export function allocPlayerContext(data: StylingContext): PlayerContext {
  return (data[StylingIndex.PlayerContext] = [
    PlayerIndex.SinglePlayerBuildersStartPosition,
    null,
    null,
    null,
    null,
...
export function hasStyling(attrs: TAttributes): boolean {
  for (let i = 0; i < attrs.length; i++) {
    const attr = attrs[i];
    if (attr == AttributeMarker.Classes || attr == AttributeMarker.Styles)
      return true;
  }
  return false;
}
