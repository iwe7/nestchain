...
function queueViewHooks(def: DirectiveDef<any>, tView: TView, i: number): void {
  ...
  if (def.afterViewChecked) {
    ...
    (tView.viewCheckHooks || (tView.viewCheckHooks = [])).push(
      i,
      def.afterViewChecked,
    );
  }
}

/** Queues onDestroy hooks on TView */
function queueDestroyHooks(
  def: DirectiveDef<any>,
  tView: TView,
  i: number,
): void {
...
export function executeHooks(
  ...
  const hooksToCall =
    currentView[FLAGS] & LViewFlags.FirstLViewPass ? allHooks : checkHooks;
  if (hooksToCall) {
