...
export function getComponent<T = {}>(element: Element): T | null {
  ...
  if (context.component === undefined) {
    context.component = getComponentAtNodeIndex(
      context.nodeIndex,
      context.lView,
    );
  }
...
export function getRootContext(target: LView | {}): RootContext {
  const lViewData = Array.isArray(target)
    ? target
    : loadLContext(target)!.lView;
...
export function getDirectives(target: {}): Array<{}> {
  ...
  if (context.directives === undefined) {
    context.directives = getDirectivesAtNodeIndex(
      context.nodeIndex,
      context.lView,
      false,
    );
...
export function loadLContext(
  ...
  if (!context && throwOnNotFound) {
    throw new Error(
      ngDevMode
        ? `Unable to find context associated with ${stringify(target)}`
        : 'Invalid ng target',
    );
  }
  return context;
  ...
  if (tCleanup && lCleanup) {
    for (let i = 0; i < tCleanup.length; ) {
      ...
      if (typeof firstParam === 'string') {
        ...
        const listenerElement = (readElementValue(
          lView[secondParam],
        ) as any) as Element;
