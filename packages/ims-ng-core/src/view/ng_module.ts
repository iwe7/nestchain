  ...
  try {
    ...
    if (providerDef) {
      ...
      if (providerInstance === undefined) {
        providerInstance = data._providers[
          providerDef.index
      ...
      (injectableDef = getInjectableDef(depDef.token)) &&
      targetsModule(data, injectableDef)
    ) {
      ...
      return (data._providers[index] = _createProviderInstance(
        data,
        data._def.providersByKey[depDef.tokenKey],
      ));
...
function moduleTransitivelyPresent(
...
function targetsModule(
  ngModule: NgModuleData,
  def: InjectableDef<any>,
): boolean {
  return (
    def.providedIn != null &&
    (moduleTransitivelyPresent(ngModule, def.providedIn) ||
...
function _createProviderInstance(
  ngModule: NgModuleData,
  providerDef: NgModuleProviderDef,
): any {
  let injectable: any;
...
function _createClass(ngModule: NgModuleData, ctor: any, deps: DepDef[]): any {
  ...
  switch (len) {
    ...
    case 2:
      return new ctor(
        resolveNgModuleDep(ngModule, deps[0]),
        resolveNgModuleDep(ngModule, deps[1]),
      );
    case 3:
      return new ctor(
        resolveNgModuleDep(ngModule, deps[0]),
...
export function callNgModuleLifecycle(
  ngModule: NgModuleData,
