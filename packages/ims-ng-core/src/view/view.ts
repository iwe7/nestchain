  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if (currentParent) {
      ...
      if (node.element && node.element.template) {
        currentParent.childMatchedQueries |=
          node.element.template.nodeMatchedQueries;
      }
    } else {
    ...
    if (node.childCount > 0) {
      ...
      while (
        currentParent &&
        i === currentParent.nodeIndex + currentParent.childCount
      ) {
...
function isNgContainer(node: NodeDef): boolean {
  return (
    (node.flags & NodeFlags.TypeElement) !== 0 && node.element!.name === null
  );
}
...
function validateNode(
  ...
  if (node.query) {
    if (
      node.flags & NodeFlags.TypeContentQuery &&
  ...
  if (node.childCount) {
    const parentEnd = parent
      ? parent.nodeIndex + parent.childCount
      : nodeCount - 1;
    if (
      node.nodeIndex <= parentEnd &&
      node.nodeIndex + node.childCount > parentEnd
    ) {
      throw new Error(
...
export function createComponentView(
  ...
  if (!rendererType) {
    ...
    compRenderer = parentView.root.rendererFactory.createRenderer(
      hostElement,
      rendererType,
    );
  }
  return createView(
    ...
    compRenderer,
    parentView,
    nodeDef.element!.componentProvider,
    viewDef,
  );
}
...
function createViewNodes(view: ViewData) {
  ...
  if (isComponentView(view)) {
    ...
    renderHost = asElementData(view.parent!, hostDef!.parent!.nodeIndex)
      .renderElement;
  }
  const def = view.def;
    ...
    switch (nodeDef.flags & NodeFlags.Types) {
      case NodeFlags.TypeElement:
        ...
        if (nodeDef.flags & NodeFlags.ComponentView) {
          ...
          componentView = Services.createComponentView(
            view,
            nodeDef,
            compViewDef,
            el,
        ...
        nodeData = <ElementData>{
          ...
          componentView,
          viewContainer: null,
          template: nodeDef.element!.template
            ? createTemplateData(view, nodeDef)
            : undefined,
        };
        if (nodeDef.flags & NodeFlags.EmbeddedViews) {
          nodeData.viewContainer = createViewContainerData(
            view,
            nodeDef,
            nodeData,
          );
      ...
      case NodeFlags.TypeDirective: {
        ...
        if (nodeDef.flags & NodeFlags.Component) {
          const compView = asElementData(view, nodeDef.parent!.nodeIndex)
            .componentView;
  ...
  execQueriesAction(
    view,
    NodeFlags.TypeContentQuery | NodeFlags.TypeViewQuery,
    NodeFlags.StaticQuery,
...
export function checkAndUpdateNode(
  ...
  if (argStyle === ArgumentType.Inline) {
    return checkAndUpdateNodeInline(
      view,
      nodeDef,
      v0,
      v1,
...
function checkAndUpdateNodeInline(
  ...
  switch (nodeDef.flags & NodeFlags.Types) {
    case NodeFlags.TypeElement:
      return checkAndUpdateElementInline(
        view,
        nodeDef,
        v0,
        v1,
    ...
    case NodeFlags.TypeText:
      return checkAndUpdateTextInline(
        view,
        nodeDef,
        v0,
        v1,
    ...
    case NodeFlags.TypeDirective:
      return checkAndUpdateDirectiveInline(
        view,
        nodeDef,
        v0,
        v1,
...
function checkAndUpdateNodeDynamic(
  view: ViewData,
  nodeDef: NodeDef,
  values: any[],
): boolean {
...
function checkNoChangesQuery(view: ViewData, nodeDef: NodeDef) {
  ...
  if (queryList.dirty) {
    throw expressionChangedAfterItHasBeenCheckedError(
      ...
      `Query ${nodeDef.query!.id} not dirty`,
      `Query ${nodeDef.query!.id} dirty`,
...
function destroyViewNodes(view: ViewData) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (def.flags & NodeFlags.TypeElement) {
      ...
      def.flags & NodeFlags.TypeContentQuery ||
      def.flags & NodeFlags.TypeViewQuery
    ) {
...
function execEmbeddedViewsAction(view: ViewData, action: ViewAction) {
  ...
  for (let i = 0; i < def.nodes.length; i++) {
    ...
    if (nodeDef.flags & NodeFlags.EmbeddedViews) {
      // a leaf
      const embeddedViews = asElementData(view, i).viewContainer!
        ._embeddedViews;
  ...
  switch (action) {
    case ViewAction.CheckNoChanges:
      if ((viewState & ViewState.Destroyed) === 0) {
        if (
          (viewState & ViewState.CatDetectChanges) ===
          ViewState.CatDetectChanges
        ) {
          checkNoChangesView(view);
        } else if (viewState & ViewState.CheckProjectedViews) {
          execProjectedViewsAction(
            view,
            ViewAction.CheckNoChangesProjectedViews,
          );
        }
      }
      break;
...
function execQueriesAction(
  view: ViewData,
  queryFlags: NodeFlags,
  staticDynamicQueryFlag: NodeFlags,
  checkType: CheckType,
) {
  if (
    !(view.def.nodeFlags & queryFlags) ||
    !(view.def.nodeFlags & staticDynamicQueryFlag)
  ) {
    return;
  }
  ...
  for (let i = 0; i < nodeCount; i++) {
    ...
    if (
      !(nodeDef.childFlags & queryFlags) ||
      !(nodeDef.childFlags & staticDynamicQueryFlag)
    ) {
