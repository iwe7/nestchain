  ...
  if (def.outputs.length) {
    for (let i = 0; i < def.outputs.length; i++) {
      ...
      if (isObservable(outputObservable)) {
        ...
        view.disposables![def.outputIndex + i] = subscription.unsubscribe.bind(
          subscription,
        );
      } else {
        throw new Error(
          `@Output ${output.propName} not initialized in '${
...
export function resolveDep(
      ...
      switch (tokenKey) {
        ...
        case ElementRefTokenKey:
          return new ElementRef(
            asElementData(searchView, elDef.nodeIndex).renderElement,
          );
        ...
        default:
          ...
          if (providerDef) {
            let providerData = asProviderData(
              searchView,
              providerDef.nodeIndex,
            );
            if (!providerData) {
              providerData = {
                instance: _createProviderInstance(searchView, providerDef),
              };
  ...
  const value = startView.root.injector.get(
    depDef.token,
...
function updateProp(
  ...
  if (def.flags & NodeFlags.OnChanges) {
    changes = changes || {};
    const oldValue = WrappedValue.unwrap(
      view.oldValues[def.bindingIndex + bindingIdx],
    );
    const binding = def.bindings[bindingIdx];
...
export function callLifecycleHooksChildrenFirst(
  view: ViewData,
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    while (
      ...
      if (parent.directChildFlags & lifecycles) {
        initIndex = callElementProvidersLifecycles(
          view,
          parent,
          lifecycles,
          initIndex,
...
function callElementProvidersLifecycles(
  ...
  for (
    let i = elDef.nodeIndex + 1;
    i <= elDef.nodeIndex + elDef.childCount;
    i++
  ) {
