...
export function resolveRendererType2(
  type?: RendererType2 | null,
): RendererType2 | null {
  if (type && type.id === UNDEFINED_RENDERER_TYPE_ID) {
    // first time we see this RendererType2. Initialize it...
    const isFilled =
      (type.encapsulation != null &&
...
export function markParentViewsForCheckProjectedViews(
  view: ViewData,
  endView: ViewData,
) {
  let currView: ViewData | null = view;
...
export function dispatchEvent(
  ...
  try {
    ...
    const startView =
      nodeDef.flags & NodeFlags.ComponentView
...
export function elementEventFullName(
  target: string | null,
  name: string,
): string {
...
export function getParentRenderElement(
  ...
  if (renderParent) {
    if (
      (renderParent.flags & NodeFlags.TypeElement) === 0 ||
...
export function resolveDefinition<D extends Definition<any>>(
...
export function rootRenderNodes(view: ViewData): any[] {
  ...
  visitRootRenderNodes(
    view,
    RenderNodeAction.Collect,
    undefined,
    undefined,
    renderNodes,
  );
...
export function visitRootRenderNodes(
  ...
  if (action === RenderNodeAction.RemoveChild) {
    parentNode = view.renderer.parentNode(
      renderNode(view, view.def.lastRenderRootNode!),
    );
  }
...
export function visitSiblingRenderNodes(
  ...
  for (let i = startIndex; i <= endIndex; i++) {
    ...
    if (
      nodeDef.flags &
      (NodeFlags.TypeElement | NodeFlags.TypeText | NodeFlags.TypeNgContent)
    ) {
...
function visitRenderNode(
  ...
  if (nodeDef.flags & NodeFlags.TypeNgContent) {
    ...
    if (
      action === RenderNodeAction.RemoveChild &&
      nodeDef.flags & NodeFlags.ComponentView &&
    ...
    if (nodeDef.flags & NodeFlags.EmbeddedViews) {
      const embeddedViews = asElementData(view, nodeDef.nodeIndex)
        .viewContainer!._embeddedViews;
    ...
    if (nodeDef.flags & NodeFlags.TypeElement && !nodeDef.element!.name) {
      visitSiblingRenderNodes(
        ...
        action,
        nodeDef.nodeIndex + 1,
        nodeDef.nodeIndex + nodeDef.childCount,
        parentNode,
        nextSibling,
        target,
      );
...
export function interpolate(
  ...
  for (let i = 0; i < valueCount * 2; i = i + 2) {
    result =
      result + constAndInterp[i] + _toStringWithNull(constAndInterp[i + 1]);
  }
