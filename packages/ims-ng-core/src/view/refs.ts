...
export function getComponentViewDefinitionFactory(
...
class ComponentFactory_ extends ComponentFactory<any> {
  ...
  create(
    ...
    const componentNodeIndex = viewDef.nodes[0].element!.componentProvider!
      .nodeIndex;
...
class Injector_ implements Injector {
  ...
  get(token: any, notFoundValue: any = Injector.THROW_IF_NOT_FOUND): any {
    const allowPrivateServices = this.elDef
      ? (this.elDef.flags & NodeFlags.ComponentView) !== 0
      : false;
...
class RendererAdapter implements RendererV1 {
  ...
  setElementProperty(
    renderElement: Element | DocumentFragment,
    propertyName: string,
    propertyValue: any,
  ): void {
...
class NgModuleRef_ implements NgModuleData, InternalNgModuleRef<any> {
  ...
  get(
    ...
    return resolveNgModuleDep(
      this,
      { token: token, tokenKey: tokenKey(token), flags: flags },
      notFoundValue,
    );
  }
