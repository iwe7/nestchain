...
export interface NgModuleDefinition
  extends Definition<NgModuleDefinitionFactory> {
...
export interface NgModuleDefinitionFactory
  extends DefinitionFactory<NgModuleDefinition> {}
...
export function shiftInitState(
  view: ViewData,
  priorInitState: ViewState,
  newInitState: ViewState,
): boolean {
...
export function shouldCallLifecycleInitHook(
  ...
  if (
    (view.state & ViewState.InitState_Mask) === initState &&
    view.initIndex <= index
  ) {
 ...
 * Accessor for view.nodes, enforcing that every usage site stays monomorphic.
 */
export function asPureExpressionData(
  view: ViewData,
  index: number,
): PureExpressionData {
  return <any>view.nodes[index];
