...
export function purePipeDef(checkIndex: number, argCount: number): NodeDef {
  ...
  return _pureExpressionDef(
    NodeFlags.TypePurePipe,
    checkIndex,
    new Array(argCount + 1),
  );
}

export function pureArrayDef(checkIndex: number, argCount: number): NodeDef {
  return _pureExpressionDef(
    NodeFlags.TypePureArray,
    checkIndex,
    new Array(argCount),
  );
}

export function pureObjectDef(
  checkIndex: number,
...
export function createPureExpression(
  view: ViewData,
  def: NodeDef,
): PureExpressionData {
