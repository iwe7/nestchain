...
export function elementDef(
  ...
  for (let i = 0; i < bindings.length; i++) {
    const [bindingFlags, namespaceAndName, suffixOrSecurityContext] = bindings[
      i
    ];
  ...
  const attrs = <[string, string, string][]>fixedAttrs.map(
...
export function listenToElementOutputs(
  view: ViewData,
  compView: ViewData,
  def: NodeDef,
  el: any,
  ...
  for (let i = 0; i < def.outputs.length; i++) {
    ...
    const handleEventClosure = renderEventHandlerClosure(
      view,
      def.nodeIndex,
      elementEventFullName(output.target, output.eventName),
    );
    let listenTarget: 'window' | 'document' | 'body' | 'component' | null =
      output.target;
...
export function checkAndUpdateElementDynamic(
  view: ViewData,
  def: NodeDef,
  values: any[],
): boolean {
...
function checkAndUpdateElementValue(
  view: ViewData,
  def: NodeDef,
  bindingIdx: number,
  value: any,
  ...
  switch (binding.flags & BindingFlags.Types) {
    ...
    case BindingFlags.TypeProperty:
      const bindView =
        def.flags & NodeFlags.ComponentView &&
...
function setElementAttribute(
  ...
  let renderValue = securityContext
    ? view.root.sanitizer.sanitize(securityContext, value)
    : value;
...
function setElementStyle(
  ...
  let renderValue: string | null = view.root.sanitizer.sanitize(
