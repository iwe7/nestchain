...
function createProdServices() {
  return {
    ...
    createDebugContext: (view: ViewData, nodeIndex: number) =>
      new DebugContext_(view, nodeIndex),
    handleEvent: (
      view: ViewData,
      nodeIndex: number,
      eventName: string,
    ...
    updateDirectives: (view: ViewData, checkType: CheckType) =>
      view.def.updateDirectives(
        checkType === CheckType.CheckAndUpdate
          ? prodCheckAndUpdateNode
          : prodCheckNoChangesNode,
        view,
      ),
    updateRenderer: (view: ViewData, checkType: CheckType) =>
      view.def.updateRenderer(
        checkType === CheckType.CheckAndUpdate
          ? prodCheckAndUpdateNode
          : prodCheckNoChangesNode,
        view,
...
function createProdRootView(
  ...
  const rendererFactory: RendererFactory2 = ngModule.injector.get(
    RendererFactory2,
  );
...
function debugCreateRootView(
  ...
  const root = createRootData(
    elInjector,
    ngModule,
    new DebugRendererFactory2(rendererFactory),
    projectableNodes,
    rootSelectorOrNode,
  );
  const defWithOverride = applyProviderOverridesToView(def);
  return callWithDebugContext(DebugAction.create, createRootView, null, [
    root,
    defWithOverride,
    context,
  ]);
...
function debugCreateEmbeddedView(
  ...
  return callWithDebugContext(DebugAction.create, createEmbeddedView, null, [
    parentView,
    anchorDef,
    defWithOverride,
    context,
...
function debugCreateComponentView(
  ...
  return callWithDebugContext(DebugAction.create, createComponentView, null, [
    parentView,
    nodeDef,
    viewDef,
    hostElement,
...
function debugOverrideProvider(override: ProviderOverride) {
  ...
  if (
    typeof override.token === 'function' &&
    (injectableDef = getInjectableDef(override.token)) &&
    typeof injectableDef.providedIn === 'function'
  ) {
    providerOverridesWithScope.set(
      override.token as InjectableType<any>,
      override,
    );
  }
}

function debugOverrideComponentView(
  comp: any,
  compFactory: ComponentFactory<any>,
) {
  const hostViewDef = resolveDefinition(
    getComponentViewDefinitionFactory(compFactory),
  );
...
function applyProviderOverridesToView(def: ViewDefinition): ViewDefinition {
  ...
  const elementIndicesWithOverwrittenProviders = findElementIndicesWithOverwrittenProviders(
    def,
  );
  ...
  for (let i = 0; i < elementIndicesWithOverwrittenProviders.length; i++) {
    applyProviderOverridesToElement(
      def,
      elementIndicesWithOverwrittenProviders[i],
    );
  }
  return def;
  ...
  function findElementIndicesWithOverwrittenProviders(
    ...
    for (let i = 0; i < def.nodes.length; i++) {
      ...
      if (
        lastElementDef &&
        nodeDef.flags & NodeFlags.CatProviderNoDirective &&
  ...
  function applyProviderOverridesToElement(
    viewDef: ViewDefinition,
    elIndex: number,
  ) {
...
function applyProviderOverridesToNgModule(
    ...
    def.providers.forEach(node => {
      ...
      if (node.flags & NodeFlags.CatProviderNoDirective && override) {
        ...
        hasDeprecatedOverrides =
          hasDeprecatedOverrides || override.deprecatedBehavior;
      }
    });
  ...
  function applyProviderOverrides(def: NgModuleDefinition) {
    for (let i = 0; i < def.providers.length; i++) {
      ...
      if (override) {
        provider.flags =
          (provider.flags & ~NodeFlags.CatProviderNoDirective) | override.flags;
        provider.deps = splitDepsDsl(override.deps);
        provider.value = override.value;
      }
    }
...
function debugCheckAndUpdateNode(
  ...
  const changed = (<any>checkAndUpdateNode)(
    view,
    nodeDef,
    argStyle,
    ...givenValues,
  ...
  if (changed) {
    const values =
      argStyle === ArgumentType.Dynamic ? givenValues[0] : givenValues;
    if (nodeDef.flags & NodeFlags.TypeDirective) {
      ...
      for (let i = 0; i < nodeDef.bindings.length; i++) {
        ...
        if (binding.flags & BindingFlags.TypeProperty) {
          bindingValues[
            normalizeDebugBindingName(binding.nonMinifiedName!)
      ...
      if (!elDef.element!.name) {
        ...
        view.renderer.setValue(
          el,
          `bindings=${JSON.stringify(bindingValues, null, 2)}`,
        );
      } else {
        // a regular element.
...
function nextDirectiveWithBinding(
  view: ViewData,
  nodeIndex: number,
): number | null {
  for (let i = nodeIndex; i < view.def.nodes.length; i++) {
    ...
    if (
      nodeDef.flags & NodeFlags.TypeDirective &&
      nodeDef.bindings &&
...
function nextRenderNodeWithBinding(
  view: ViewData,
  nodeIndex: number,
): number | null {
  for (let i = nodeIndex; i < view.def.nodes.length; i++) {
...
class DebugContext_ implements DebugContext {
  ...
  private get elOrCompView() {
    ...
    return (
      asElementData(this.elView, this.elDef.nodeIndex).componentView ||
      this.view
    );
  }
  ...
  get providerTokens(): any[] {
    ...
    if (this.elDef) {
      for (
        let i = this.elDef.nodeIndex + 1;
        i <= this.elDef.nodeIndex + this.elDef.childCount;
        i++
      ) {
  ...
  get renderNode(): any {
    return this.nodeDef.flags & NodeFlags.TypeText
      ? renderNode(this.view, this.nodeDef)
...
function collectReferences(
  ...
  for (let refName in nodeDef.references) {
    references[refName] = getQueryValue(
      view,
      nodeDef,
      nodeDef.references[refName],
    );
  }
}
...
export function getCurrentDebugContext(): DebugContext | null {
  return _currentView
    ? new DebugContext_(_currentView, _currentNodeIndex)
    : null;
}
...
export class DebugRenderer2 implements Renderer2 {
  ...
  debugContextFactory: (
    nativeElement?: any,
  ) => DebugContext | null = getCurrentDebugContext;
