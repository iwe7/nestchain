...
export function dirtyParentQueries(view: ViewData) {
    ...
    for (let i = 0; i <= end; i++) {
      ...
      if (
        (nodeDef.flags & NodeFlags.TypeElement &&
          i + nodeDef.childCount < tplDef.nodeIndex) ||
  ...
  if (view.def.nodeFlags & NodeFlags.TypeViewQuery) {
    for (let i = 0; i < view.def.nodes.length; i++) {
      const nodeDef = view.def.nodes[i];
      if (
        nodeDef.flags & NodeFlags.TypeViewQuery &&
        nodeDef.flags & NodeFlags.DynamicQuery
      ) {
        asQueryList(view, i).setDirty();
      }
...
export function checkAndUpdateQuery(view: ViewData, nodeDef: NodeDef) {
  ...
  if (nodeDef.flags & NodeFlags.TypeContentQuery) {
    ...
    directiveInstance = asProviderData(view, nodeDef.parent!.nodeIndex)
      .instance;
...
function calcQueryValues(
  ...
  for (let i = startIndex; i <= endIndex; i++) {
    ...
    if (
      ...
      if (
        (nodeDef.childMatchedQueries & queryDef.filterId) ===
        queryDef.filterId
      ) {
        ...
        for (let k = 0; k < embeddedViews.length; k++) {
          ...
          if (dvc && dvc === elementData) {
            calcQueryValues(
              embeddedView,
              0,
              embeddedView.def.nodes.length - 1,
              queryDef,
              values,
            );
          }
      ...
      if (projectedViews) {
        for (let k = 0; k < projectedViews.length; k++) {
          ...
          calcQueryValues(
            projectedView,
            0,
            projectedView.def.nodes.length - 1,
            queryDef,
            values,
          );
        }
    ...
    switch (queryValueType) {
      ...
      case QueryValueType.ElementRef:
        return new ElementRef(
          asElementData(view, nodeDef.nodeIndex).renderElement,
        );
