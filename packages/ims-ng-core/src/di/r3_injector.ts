...
export class R3Injector {
  ...
  private processInjectorType(
    ...
    const ngModule =
      (def == null &&
        (defOrWrappedDef as InjectorTypeWithProviders<any>).ngModule) ||
      undefined;
    ...
    const defType: InjectorType<any> =
      ngModule === undefined
        ? (defOrWrappedDef as InjectorType<any>)
        : ngModule;
    ...
    if (ngDevMode && parents.indexOf(defType) !== -1) {
      ...
      throw new Error(
        `Circular dependency in DI detected for type ${defName}. Dependency path: ${parents
          .map(defType => stringify(defType))
          .join(' > ')} > ${defName}.`,
      );
    }
    ...
    const providers =
      (ngModule !== undefined &&
        (defOrWrappedDef as InjectorTypeWithProviders<any>).providers) ||
      EMPTY_ARRAY;
    ...
    const ngModuleType = (defOrWrappedDef as InjectorTypeWithProviders<any>)
      .ngModule;
    deepForEach(providers, provider =>
      this.processProvider(provider, ngModuleType, providers),
    );
  }
  ...
  private hydrate<T>(token: Type<T> | InjectionToken<T>, record: Record<T>): T {
    if (record.value === CIRCULAR) {
      throw new Error(
        `Cannot instantiate cyclic dependency! ${stringify(token)}`,
      );
  ...
  private injectableDefInScope(def: InjectableDef<any>): boolean {
    if (!def.providedIn) {
      ...
      return (
        def.providedIn === 'any' ||
        (def.providedIn === 'root' && this.isRootInjector)
      );
    } else {
...
function injectableDefOrInjectorDefFactory(
  ...
  if (injectableDef === null) {
    ...
    if (injectorDef !== null) {
      ...
      throw new Error(
        `Token ${stringify(token)} is missing an ngInjectableDef definition.`,
      );
...
function providerToRecord(
  ...
  let factory: (() => any) | undefined = providerToFactory(
    provider,
    ngModuleType,
    providers,
  );
  ...
  if (isTypeProvider(provider)) {
    ...
    if (isValueProvider(provider)) {
      ...
      if (!classRef) {
        ...
        throw new Error(
          `Invalid provider for the NgModule '${stringify(ngModuleType)}'` +
            ngModuleDetail,
        );
      }
      if (hasDeps(provider)) {
...
function couldBeInjectableType(
  value: any,
): value is Type<any> | InjectionToken<any> {
  return (
