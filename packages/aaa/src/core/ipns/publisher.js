...
class IpnsPublisher {
    ...
    ipns.embedPublicKey(publicKey, record, (err, embedPublicKeyRecord) => {
      ...
      series(
        [
          ...
          cb =>
            this._publishPublicKey(keys.routingPubKey, publicKey, peerId, cb),
        ],
        err => {
          if (err) {
            log.error(err);
    ...
    this._datastore.get(ipns.getLocalKey(peerId.id), (err, dsVal) => {
      if (err) {
        if (err.code !== 'ERR_NOT_FOUND') {
          const errMsg = `unexpected error getting the ipns record ${
            peerId.id
          } from datastore`;

          log.error(errMsg);
          return callback(
            errcode(new Error(errMsg), 'ERR_UNEXPECTED_DATASTORE_RESPONSE'),
          );
        }
  ...
  _updateOrCreateRecord(privKey, value, validity, peerId, callback) {
    ...
    this._getPublished(peerId, getPublishedOptions, (err, record) => {
      if (err) {
        if (err.code !== 'ERR_NOT_FOUND') {
          const errMsg = `unexpected error when determining the last published IPNS record for ${
            peerId.id
          }`;

          log.error(errMsg);
          return callback(
            errcode(new Error(errMsg), 'ERR_DETERMINING_PUBLISHED_RECORD'),
          );
        }
      }
      ...
      ipns.create(privKey, value, seqNumber, validity, (err, entryData) => {
        if (err) {
          ...
          log.error(errMsg);
          return callback(
            errcode(new Error(errMsg), 'ERR_CREATING_IPNS_RECORD'),
          );
        }
        ...
        this._datastore.put(ipns.getLocalKey(peerId.id), data, (err, res) => {
          if (err) {
            ...
            log.error(errMsg);
            return callback(
              errcode(new Error(errMsg), 'ERR_STORING_IN_DATASTORE'),
            );
          }
