...
class IpnsResolver {
  ...
  resolve(name, options, callback) {
    ...
    options = options || {};
    const recursive =
      options.recursive && options.recursive.toString() === 'true';
  ...
  resolver(name, depth, callback) {
    ...
    if (depth === 0) {
      ...
      log.error(errMsg);
      return callback(
        errcode(new Error(errMsg), 'ERR_RESOLVE_RECURSION_LIMIT'),
      );
    }
    ...
    this._routing.get(routingKey.toBuffer(), (err, record) => {
      if (err) {
        if (err.code !== 'ERR_NOT_FOUND') {
          ...
          log.error(errMsg);
          return callback(
            errcode(new Error(errMsg), 'ERR_UNEXPECTED_ERROR_GETTING_RECORD'),
          );
        }
      ...
      try {
        ...
        log.error(errMsg);
        return callback(
          errcode(new Error(errMsg), 'ERR_INVALID_RECORD_RECEIVED'),
        );
      }
      ...
      this._routing.get(routingKey.toBuffer(), (err, pubKey) => {
        if (err) {
          if (err.code !== 'ERR_NOT_FOUND') {
            const errMsg = `unexpected error getting the public key for the ipns record ${
              peerId.id
            }`;
        ...
        try {
          ...
          log.error(errMsg);
          return callback(
            errcode(new Error(errMsg), 'ERR_INVALID_PUB_KEY_RECEIVED'),
          );
        }
