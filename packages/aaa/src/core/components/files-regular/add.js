...
module.exports = function(self) {
  const add = promisify((data, options, callback) => {
    ...
    const isBufferOrStream = obj =>
      Buffer.isBuffer(obj) || isStream.readable(obj) || isSource(obj);
    ...
    if (!ok) {
      return callback(
        new Error(
          'invalid input: expected buffer, readable stream, pull stream, object or array of objects',
        ),
      );
    }
